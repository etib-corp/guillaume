...
{
    // Map Component::InteractionState to Button::ButtonState
...
{
    // Log button-specific state changes
    const char* state_names[] = {"NORMAL", "HOVERED", "PRESSED"};
    LOG_INFO_F("Button '{}' [{}] state change: {} → {}", 
               _label, 
               get_identifier(),
...
void Button::_on_click_detected()
{
    ...
    // Call button-specific click callback
    if (_on_button_click) {
