...
SimpleWindow::SimpleWindow()
  ...
  _sdlWindow = SDL_CreateWindow(_title.c_str(), 800, 600, SDL_WINDOW_RESIZABLE);
  if (!_sdlWindow) {
    _logger.error(std::string("SDL_CreateWindow failed: ") + SDL_GetError());
    SDL_Quit();
  ...
  if (!sdlRenderer) {
    _logger.error(std::string("SDL_CreateRenderer failed: ") + SDL_GetError());
...
void SimpleWindow::show(void) {
  ...
  if (_sdlWindow) {
    if (!SDL_ShowWindow(_sdlWindow)) {
      _logger.error(std::string("SDL_ShowWindow failed: ") + SDL_GetError());
    }
  }
...
void SimpleWindow::hide(void) {
  ...
  if (_sdlWindow) {
    if (!SDL_HideWindow(_sdlWindow)) {
      _logger.error(std::string("SDL_HideWindow failed: ") + SDL_GetError());
    }
  }
...
void SimpleWindow::restore(void) {
  ...
  if (_sdlWindow) {
    if (!SDL_RestoreWindow(_sdlWindow)) {
      _logger.error(std::string("SDL_RestoreWindow failed: ") + SDL_GetError());
    }
  }
...
bool SimpleWindow::sync(void) {
  if (_sdlWindow) {
    if (!SDL_SyncWindow(_sdlWindow)) {
      _logger.error(std::string("SDL_SyncWindow failed: ") + SDL_GetError());
      return false;
    }
  }
