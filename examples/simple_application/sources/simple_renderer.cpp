...
void SimpleRenderer::setSDLRenderer(SDL_Renderer *renderer) {
  ...
  if (_sdlRenderer) {
    ...
    SDL_SetRenderDrawColor(_sdlRenderer, _drawColor.red(), _drawColor.green(),
                           _drawColor.blue(), _drawColor.alpha());
    // Set initial scale
...
void SimpleRenderer::clear(void) {
  if (_sdlRenderer) {
    if (!SDL_RenderClear(_sdlRenderer)) {
      _logger.error(std::string("SDL_RenderClear failed: ") + SDL_GetError());
    }
  } else {
...
void SimpleRenderer::present(void) {
  if (_sdlRenderer) {
    if (!SDL_RenderPresent(_sdlRenderer)) {
      _logger.error(std::string("SDL_RenderPresent failed: ") + SDL_GetError());
    }
  } else {
...
void SimpleRenderer::drawPoint(utility::Vector<std::size_t, 2> point) {
  if (_sdlRenderer) {
    if (!SDL_RenderPoint(_sdlRenderer, static_cast<float>(point[0]),
      ...
      _logger.error(std::string("SDL_RenderPoint failed: ") + SDL_GetError());
    }
  } else {
...
void SimpleRenderer::drawLine(utility::Vector<std::size_t, 2> start,
  ...
  if (_sdlRenderer) {
    if (!SDL_RenderLine(_sdlRenderer, static_cast<float>(start[0]),
      ...
      _logger.error(std::string("SDL_RenderLine failed: ") + SDL_GetError());
    }
  } else {
...
void SimpleRenderer::drawRect(utility::Rectangle<std::size_t> rectangle) {
  if (_sdlRenderer) {
    ...
    if (!SDL_RenderRect(_sdlRenderer, &rect)) {
      _logger.error(std::string("SDL_RenderRect failed: ") + SDL_GetError());
    }
  } else {
    _logger.debug(
...
bool SimpleRenderer::flush(void) {
  if (_sdlRenderer) {
    if (!SDL_FlushRenderer(_sdlRenderer)) {
      _logger.error(std::string("SDL_FlushRenderer failed: ") + SDL_GetError());
      return false;
    }
  } else {
